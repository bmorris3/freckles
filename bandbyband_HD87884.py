...
comparisons = [['HD145675', '2018-06-01T04:54:33.780'],  # 5282 Photosphere template
               ['HD145742', '2018-06-01T05:09:53.040']]  # 4851 Spot template
...
for time in times:
    if time not in results or force_refit:
        ...
        for inds, band in zip(target_slices.wavelength_splits, bands):
            ...
            plt.savefig('plots_{0}/{0}_{1}_{2}.pdf'.format(star, int(band.core.value),
                                                           time.replace(':', '_')),
                        bbox_inches='tight')
            plt.close()
            ...
            plt.savefig('plots_{0}/{0}_{1}_{2}_fit.pdf'.format(star, int(band.core.value),
                                                           time.replace(':', '_')),
                        bbox_inches='tight')
            plt.close()
            # fig.close()
